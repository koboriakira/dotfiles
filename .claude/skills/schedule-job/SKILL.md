...
| | schedule-job | `/loop` コマンド |
|---|---|---|
| 永続性 | cron/launchd で永続化（再起動後も実行） | セッション内のみ（セッション終了で停止） |
| 用途 | 日次・週次の定期バッチ処理 | セッション中の一時的なモニタリング・ポーリング |
| 例 | 毎朝7時にブログ生成、日次スキャン | デプロイ完了を5分おきに確認、テスト結果の定期チェック |

**判断基準:** セッションをまたいで繰り返し実行する必要があるなら `schedule-job`、今のセッション中だけ監視したいなら `/loop` を使う。
