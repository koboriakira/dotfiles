...
```bash
gws gmail +triage --max 50 --query 'is:unread newer_than:1h' --format json
```
...
すべてのメールをアーカイブする。上記「要確認」に該当するものは「要確認」ラベルを付けてからアーカイブする。

### 4. 「要確認」ラベルIDの取得

アーカイブ実行前に「要確認」ラベルの ID を取得する。

```bash
gws gmail users labels list --params '{"userId": "me"}'
```

レスポンスの `labels` 配列から `name` が `"要確認"` のものの `id` を取得する。ラベルが存在しない場合は作成する。

```bash
gws gmail users labels create --params '{"userId": "me"}' --json '{"name": "要確認"}'
```

### 5. アーカイブ実行

ユーザー確認なしで自動的にアーカイブを実行する。
...
**要確認アーカイブ（要確認ラベルを付けてアーカイブ）:**

```bash
gws gmail users messages modify --params '{"userId": "me", "id": "<MESSAGE_ID>"}' --json '{"addLabelIds": ["<要確認ラベルID>"], "removeLabelIds": ["INBOX", "UNREAD"]}'
```
...
**通知対象:**
- 要確認ラベルを付けてアーカイブしたメール（全件）
- それ以外でアーカイブしたメールのうち重要度が比較的高いもの（決済通知、サービス障害、期限付きの案内など）

**通知フォーマット:**
```
📬 メールトリアージ完了（アーカイブ: N件 / うち要確認: M件）
